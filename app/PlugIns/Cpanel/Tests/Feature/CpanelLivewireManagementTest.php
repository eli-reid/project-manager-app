...
use App\PlugIns\Cpanel\Jobs\SyncEmailAccountsJob;
use App\PlugIns\Cpanel\Livewire\Admin\EmailAccounts\Create as EmailAccountsCreate;
use App\PlugIns\Cpanel\Livewire\Admin\EmailAccounts\Index as EmailAccountsIndex;
use App\PlugIns\Cpanel\Livewire\Admin\EmailAccounts\Show as EmailAccountsShow;
use App\PlugIns\Cpanel\Livewire\Admin\EmailManagement\Dashboard;
use App\PlugIns\Cpanel\Livewire\Admin\EmailManagement\DomainForwarders as EmailManagementDomainForwarders;
...
it('launches webmail from detail screen', function () {
    ...
    Http::assertSent(function (Request $request): bool {
        if (! str_starts_with($request->url(), 'https://cpanel.example.test:2083/execute/Session/create_webmail_session_for_mail_user')) {
            return false;
        }

        parse_str(parse_url($request->url(), PHP_URL_QUERY) ?: '', $query);

        return ($query['login'] ?? null) === 'jane'
            && ($query['domain'] ?? null) === 'example.test';
    });
});
