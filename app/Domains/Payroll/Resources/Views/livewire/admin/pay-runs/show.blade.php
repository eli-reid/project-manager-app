<section class="w-full space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="space-y-1">
            <flux:heading size="xl">Pay Run Details</flux:heading>
        ...
        <div class="flex flex-wrap items-center gap-2">
            <flux:button icon="arrow-left" :href="route('admin.payroll.runs.index')" wire:navigate size="sm">
                Back to Runs
            </flux:button>

            @can('payroll-runs.approve')
                @if ($run->status->value === 'preview')
                    <flux:button wire:click="approve" icon="check" size="sm" wire:loading.attr="disabled">
                        Approve
                    </flux:button>
                @endif
            @endcan
            ...
            @can('payroll-runs.finalize')
                @if ($run->status->value === 'approved')
                    <flux:button wire:click="finalize" icon="lock-closed" size="sm" wire:loading.attr="disabled">
                        Finalize
                    </flux:button>
                @endif
            @endcan
            ...
            @can('payroll-runs.void')
                @if ($run->status->value === 'finalized')
                    <flux:button wire:click="voidRun" icon="x-circle" size="sm" wire:loading.attr="disabled">
                        Void Run
                    </flux:button>
                @endif
            @endcan
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Status</flux:text>
            <div class="mt-3">
                <flux:badge>{{ $run->status->label() }}</flux:badge>
            </div>
        </div>
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Pay Date</flux:text>
            <flux:heading size="lg" class="mt-2">{{ optional($run->pay_date)->format('M j, Y') }}</flux:heading>
        </div>
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Employees</flux:text>
            <flux:heading size="lg" class="mt-2">{{ (int) $run->employee_count }}</flux:heading>
        </div>
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Gross</flux:text>
            <flux:heading size="lg" class="mt-2">${{ number_format((float) $run->total_gross, 2) }}</flux:heading>
        </div>
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Net</flux:text>
            <flux:heading size="lg" class="mt-2">${{ number_format((float) $run->total_net, 2) }}</flux:heading>
        </div>
    </div>

    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        <div class="border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
            <flux:heading size="lg">Payroll Statements</flux:heading>
            <flux:text class="mt-1 text-zinc-600 dark:text-zinc-400">
                Review the employee statements included in this pay run before moving the workflow forward.
            </flux:text>
        </div>
