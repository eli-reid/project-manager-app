<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            @if ($embeddedProject)
                <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ $rfiCount }} {{ Str::plural('RFI', $rfiCount) }} on this project.</p>
            @else
    ...
    @if ($embeddedProject && $isCreateMode)
        <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input id="rfi-subject" type="text" wire:model="subject" class="mt-1 block w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" placeholder="Brief summary of your question" />
                    @error('subject') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="rfi-body" class="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Description</label>
                    <textarea id="rfi-body" wire:model="body" rows="4" class="mt-1 block w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" placeholder="Provide additional context..."></textarea>
                    @error('body') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div class="w-48">
                    <label for="rfi-due-date" class="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Due Date</label>
                    <input id="rfi-due-date" type="date" wire:model="dueDate" class="mt-1 block w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-zinc-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                    ...
                    @else
                        <div class="mt-3 space-y-2">
                            @foreach ($availableDocuments as $document)
                                @php $documentId = (string) $document->id; @endphp
                ...
                <div class="flex gap-2">
                    <button wire:click="submitRfi" wire:loading.attr="disabled" class="rounded-md bg-zinc-900 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-700 disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300">Submit RFI</button>
                    <button wire:click="cancelCreate" class="rounded-md border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800">Cancel</button>
                </div>
            </div>
        </div>
    @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    @forelse ($rfis as $rfi)
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.rfis.show', $rfi) }}" wire:navigate class="rounded-md border border-zinc-300 px-2.5 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
                                    Review
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ $embeddedProject ? 6 : 7 }}" class="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">
                                No RFIs found.
                            </td>
                        </tr>
                    @endforelse
