<div class="mx-auto w-full max-w-4xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    <div>
        ...
        <flux:text class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
            {{ __('Create a draft, keep editing as needed, and submit when ready for approval.') }}
        </flux:text>
    </div>
    ...
    <form wire:submit="saveAsDraft" class="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div>
            ...
            <div class="grid gap-2 rounded-lg border border-zinc-200 p-3 md:grid-cols-2 dark:border-zinc-700">
                @forelse ($employees as $employeeName)
                    <label wire:key="onsite-employee-{{ md5($employeeName) }}" class="inline-flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 border-t border-zinc-200 pt-4 dark:border-zinc-700">
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('dailies.index') }}" class="rounded-md border border-zinc-300 px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800" wire:navigate>{{ __('Cancel') }}</a>
                ...
                @if ($canDelete)
                    <button
                        ...
                        class="rounded-md border border-rose-300 px-3 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50 dark:border-rose-700 dark:text-rose-300 dark:hover:bg-rose-900/20"
                    >
                        {{ __('Delete') }}
                    </button>
                @endif
            ...
            <div class="flex flex-wrap gap-2">
                <button type="submit" class="rounded-md border border-zinc-300 px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
                    {{ $isEdit ? __('Update Draft') : __('Save Draft') }}
                </button>
                ...
                @if ($canSubmit)
                    <button
                        ...
                        class="rounded-md bg-zinc-900 px-3 py-2 text-sm font-semibold text-white hover:bg-zinc-700 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300"
                    >
                        {{ __('Save & Submit') }}
                    </button>
                @endif
