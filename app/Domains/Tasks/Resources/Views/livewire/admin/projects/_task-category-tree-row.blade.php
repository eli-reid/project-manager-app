...
<tr
    ...
    @contextmenu.prevent.stop="openContextMenu($event, {
        ...
        canCreateCategory: @js($canCreateTaskCategory),
...
>
    <td class="px-3 py-2 align-top text-sm font-semibold text-zinc-900 dark:text-zinc-100" @style(["padding-left: {$categoryIndent}px"] )>
        <div class="inline-flex items-center gap-2">
            ...
            @if ($editingCategoryName === $categoryId && $canUpdateTaskCategory)
                <form wire:submit="saveCategoryName" @click.stop class="flex items-center gap-1">
                    <input
                        type="text"
    ...
    <td class="px-3 py-2 align-top text-right">
        <div class="relative inline-block text-left" x-data="buildMenuState(150)" @click.stop @click.away="closeMenu()">
            <button type="button" @click.stop="toggleMenu($event)" class="inline-flex h-7 w-7 items-center justify-center rounded-md border border-zinc-300 text-zinc-500 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800" aria-label="Category actions">
            ...
            <div x-show="open" x-cloak @click.stop class="fixed z-30 w-44 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-lg dark:border-zinc-700 dark:bg-zinc-900" :style="menuStyle">
                <button type="button" @click.stop="open = false" wire:click="startInlineTaskForm('{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Quick Add Task</button>
                <button type="button" @click.stop="open = false" wire:click="startInlineCategoryForm('{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Quick Add Subcategory</button>
                @if ($canUpdateTaskCategory)
                    <button type="button" @click.stop="open = false" wire:click="startEditCategoryName('{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Rename Category</button>
                @endif
                <button type="button" @click.stop="open = false" wire:click="copyCategoryFrom('{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Copy Category</button>
                <button type="button" @click.stop="open = false; showCopyModal = true" wire:click="$set('copySourceCategoryId', '{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Copy Category Tasks</button>
                @if ($canCreateTaskTemplate)
                    <button type="button" @click.stop="open = false" wire:click="startSaveCategoryAsTemplate('{{ $categoryId }}')" class="block w-full px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800">Save as Template</button>
                @endif
