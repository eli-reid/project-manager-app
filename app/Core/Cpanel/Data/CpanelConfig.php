...
class CpanelConfig
{
    public function __construct(
        ...
        public readonly bool $queueWriteOperations = false,
        public readonly int $idempotencyTtlSeconds = 120,
        public readonly int $queueTries = 3,
        public readonly string $queueBackoff = '10,30,60',
        public readonly int $failureThreshold = 5,
        public readonly int $cooldownSeconds = 300,
        public readonly string $telemetryKeyPrefix = 'cpanel.telemetry',
    ...
    public static function fromServicesConfig(?array $config): self
    {
        ...
        return new self(
            ...
            queueWriteOperations: (bool) ($config['queue_write_operations'] ?? false),
            idempotencyTtlSeconds: (int) ($config['idempotency_ttl_seconds'] ?? 120),
            queueTries: (int) ($config['queue_tries'] ?? 3),
            queueBackoff: (string) ($config['queue_backoff'] ?? '10,30,60'),
            failureThreshold: (int) ($config['failure_threshold'] ?? 5),
            cooldownSeconds: (int) ($config['cooldown_seconds'] ?? 300),
            telemetryKeyPrefix: (string) ($config['telemetry_key_prefix'] ?? 'cpanel.telemetry'),
