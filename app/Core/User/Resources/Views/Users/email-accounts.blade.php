<!-- filepath: c:\laragon\www\project-manager\resources\views\admin\users\email-accounts.blade.php -->
<x-app-layout>
    <x-slot name="header">
                                ...
                                @forelse($emails as $email)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ number_format($email['quota'] / 1024, 2) }} MB
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                                                <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ $email['usage_percentage'] }}%"></div>
                                            </div>
                                            <span class="text-xs mt-1 block">
                                                {{ number_format($email['usage'] / 1024, 2) }} MB ({{ $email['usage_percentage'] }}%)
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            @if($email['suspended'])
