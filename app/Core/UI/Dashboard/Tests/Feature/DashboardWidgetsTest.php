...
it('renders dashboard widgets with consistent span classes', function (): void {
    ...
    $this->actingAs($user)
        ...
        ->assertSee('fixed inset-y-0 left-0 z-30', false)
        ->assertSee('data-test="dashboard-panel-overview-link"', false)
        ->assertSee('data-test="sidebar-menu-button"', false)
        ->assertSee('Operational snapshot across all registered dashboard widgets.')
        ->assertSee('placeholder="Search panels"', false)
        ->assertSee('bg-white/72', false)
        ->assertSee('dark:bg-zinc-950/45', false)
